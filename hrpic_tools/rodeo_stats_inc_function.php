<?php
	function f_calculate_date_periode($im_option, $im_semaine_choisi_from, $im_semaine_choisi_to, &$ch_mois_choisi_from, &$ch_an_choisi_from, &$ch_mois_choisi_to, &$ch_an_choisi_to, &$ch_date_from, &$ch_date_to) {
//		echo "an_choisi_from:".$ch_an_choisi_from." semaine_choisi_from:".$im_semaine_choisi_from."<br />";
//		echo "an_choisi_to:".$ch_an_choisi_to." semaine_choisi_to:".$im_semaine_choisi_to."<br />";
		
		$ch_date_from = new DateTime('now');
		$ch_date_from->setTimestamp(strtotime($ch_date_from->format("m/d/y 00:00:01")));
//		echo "im_option = $im_option<br />";
		if ($im_option == 9) {
			$ch_date_to = clone $ch_date_from;
			$ch_date_from->setTimestamp(strtotime($ch_mois_choisi_from.'/01/'.$ch_an_choisi_from.' 00:00:01'));
			//$ch_date_from->modify('First day of month');
			$ch_date_to->setTimestamp(strtotime($ch_mois_choisi_to.'/01/'.$ch_an_choisi_to.' 23:59:59'));
			//$ch_date_to->modify('First day of next month');
			$ch_date_to->modify('Last day of this month');
			//$ch_date_to->modify('-1 day');
		} else {
			switch ($im_option) {
				case 0:		//semaine prÃ©cÃ©dente
					...
					$ch_date_to->setTimestamp(strtotime($ch_date_to->format("m/d/y 23:59:59")));
					break;
				case 1:		//mois prÃ©cÃ©dent
					$ch_date_to = clone $ch_date_from;
					$ch_date_from->modify('First day of previous month');
					$ch_date_from->setTimestamp(strtotime($ch_date_from->format("m/d/y 00:00:01")));
					//$ch_date_to->modify('First day of month');
					$ch_date_to->modify('Last day of previous month');
					//$ch_date_to->modify('-1 day');
					$ch_date_to->setTimestamp(strtotime($ch_date_to->format("m/d/y 23:59:59")));
					break;
				case 2:		//trimestre prÃ©cÃ©dent
					$ch_date_to = clone $ch_date_from;;
					switch ($ch_date_from->format('m')) {
						case '01':
						case '02':
						case '03':
							$ch_date_from->setTimestamp(strtotime('09/01/'.($ch_date_from->format('Y') - 1).' 00:00:01'));
							$ch_date_to->setTimestamp(strtotime('12/31/'.($ch_date_from->format('Y') - 1).' 23:59:59'));
							break;
						case '04':
						case '05':
						case '06':
							$ch_date_from->setTimestamp(strtotime('01/01/'.$ch_date_from->format('Y').' 00:00:01'));
							$ch_date_to->setTimestamp(strtotime('03/31/'.$ch_date_from->format('Y').' 23:59:59'));
							break;
						case '07':
						case '08':
						case '09':
							$ch_date_from->setTimestamp(strtotime('04/01/'.$ch_date_from->format('Y').' 00:00:01'));
							$ch_date_to->setTimestamp(strtotime('06/30/'.$ch_date_from->format('Y').' 23:59:59'));
							break;
						case '10':
						case '11':
						case '12':
							$ch_date_from->setTimestamp(strtotime('07/01/'.$ch_date_from->format('Y').' 00:00:01'));
							$ch_date_to->setTimestamp(strtotime('09/30/'.$ch_date_from->format('Y').' 23:59:59'));
							break;
					}
					break;
				case 3:		//annÃ©e fiscale
					$ch_date_to = clone $ch_date_from;
					if($ch_date_from->format('m') < 4) {
						$ch_date_from->setTimestamp(strtotime('04/01/'.($ch_date_from->format('Y') - 1).' 00:00:01'));
						$ch_date_to->setTimestamp(strtotime('03/31/'.$ch_date_to->format('Y').' 23:59:59'));
					} else {
						$ch_date_from->setTimestamp(strtotime('04/01/'.$ch_date_from->format('Y').' 00:00:01'));
						$ch_date_to->setTimestamp(strtotime('03/31/'.($ch_date_to->format('Y') + 1).' 23:59:59'));
					}
					break;
				case 4:		//mois actuel
		 			$ch_date_to = clone $ch_date_from;
					$ch_date_from->modify('First day of this month');
					$ch_date_from->setTimestamp(strtotime($ch_date_from->format("m/d/y 00:00:01")));
					$ch_date_to->modify('Last day of this month');
					$ch_date_to->setTimestamp(strtotime($ch_date_to->format("m/d/y 23:59:59")));
					break;
				case 5:		//semaine
		 			$ch_date_from->setISODate($ch_an_choisi_from, $im_semaine_choisi_from);
					$ch_date_from->setTimestamp(strtotime($ch_date_from->format("m/d/y 00:00:01")));
		 			$ch_date_to = clone $ch_date_from;
		 			$ch_date_to->setISODate($ch_an_choisi_to, $im_semaine_choisi_to);
					$ch_date_to->modify('+6 days');
					$ch_date_to->setTimestamp(strtotime($ch_date_to->format("m/d/y 23:59:59")));
//					gestion des cas oÃ¹ la fin de semaine est sur N+1 mais oÃ¹ on a demandÃ© annÃ©e max = annÃ©e N
					if($ch_date_to->format('Y') > $ch_an_choisi_to){
						$ch_date_to->modify('First day of this month');
						$ch_date_to->modify('-1 day');
					}
					break;
			}
...
//		echo "date_from:".$ch_date_from->format('d-m-y h:i:s')." date_to:".$ch_date_to->format('d-m-y h:i:s')."<br />";
...
//  	La premiÃ¨re semaine ne contient pas forcÃªment le premier janvier
		$im_jour1_semaine1 = 1;
...
//		$ch_time_mois_debut->setTimestamp(strtotime($im_mois_periode.'/'.$im_jour1_semaine1.'/'.$im_an_periode));
		$ch_time_mois_debut->setTimestamp(strtotime($im_mois_periode.'/01/'.$im_an_periode));
		...
		$ch_time_mois_debut->setTimestamp(strtotime($ch_time_mois_debut->format("m/d/y 00:00:01")));
		$ch_time_mois_fin->setTimestamp(strtotime($ch_time_mois_fin->format("m/d/y 23:59:59")));
	}
...
//		/^ticket ?    <= chaÃ®ne dÃ©butant par "ticket", Ã©ventuellement suivie d'un espace PUIS
//		[xnz0-9]{3,} <= rÃ©pÃ©tition d'au moins 3 chiffres ou 3 "x" ou 3 "n" ou 3 "z"
		if(preg_match("/^ticket ? ?[xnz0-9]{3,}/i", $im_valeur))
			$im_valeur = preg_replace("/^ticket ? ?/i", "", $im_valeur);
//		/^mantis ?    <= chaÃ®ne dÃ©butant par "ticket", Ã©ventuellement suivie d'un espace PUIS
//		[xnz0-9]{3,} <= rÃ©pÃ©tition d'au moins 3 chiffres ou 3 "x" ou 3 "n" ou 3 "z"
		if(preg_match("/^mantis ? ?[xnz0-9]{3,}/i", $im_valeur))
			$im_valeur = preg_replace("/^mantis ? ?/i", "", $im_valeur);
//		/^fiche ?    <= chaÃ®ne dÃ©butant par "ticket", Ã©ventuellement suivie d'un espace PUIS
//		[xnz0-9]{3,} <= rÃ©pÃ©tition d'au moins 3 chiffres ou 3 "x" ou 3 "n" ou 3 "z"
		if(preg_match("/^fiche ? ?[xnz0-9]{3,}/i", $im_valeur))
			$im_valeur = preg_replace("/^fiche ? ?/i", "", $im_valeur);
//		/^hrx? ?    <= chaÃ®ne dÃ©butant par "hrx" ou "hr", Ã©ventuellement suivie d'un espace PUIS
//		[xnz0-9]{3,} <= rÃ©pÃ©tition d'au moins 3 chiffres ou 3 "x" ou 3 "n" ou 3 "z"
