...
function ProjektOption(Index,pNum,lNum,tNum, disabled, d1,d2,d3,d4,d5,d6,d7,theme,com,evolLibelle,nbRows) {
	with (document) {
		...
		writeln("<td><input type='text' "+disabled+" name='commentaire"+Index+"' value='"+com+"' "+
		        ...
		        " onblur='LostFokus3(this)'>");
		writeln("<td><input type='text' name='evolLibelle"+Index+"' value='"+evolLibelle+"' class='textOnly' readonly></td></tr>");
	}
}
...
function changeThemeFieldType(index, activiteId) {
	if(activiteId == 2) {
		var element = document.getElementById("theme"+index);
...
function activityIisAnEvol(activiteId) {
...
//	/Bug Internet Explorer
	...
	if (liste_activite_evolution.indexOf(activiteid) > -1)
		requestEvolID(ajaxCheckEvolId, e, projid, evolid, ligne);
	else
		checkWeekData(e, ligne + 1);
	return;
}

function requestEvolID(callback, e, projid, evolid, ligne) {
	var xhr = getXhr();
	// On défini ce qu'on va faire quand on aura la réponse
	xhr.onreadystatechange = function(){
		// On ne fait quelque chose que si on a tout reçu et que le serveur est ok
		if(xhr.readyState == 4 && xhr.status == 200)
			callback(xhr.responseText, e, evolid, ligne);
	}
...
function ajaxCheckEvolId(status, e, evolid, ligne) {
	switch(status) {
		case '0':
			checkWeekData(e, ligne + 1);
			return true;
