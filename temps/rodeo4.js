...
function changeMenu(Index) {
	...
	if (projValue == 0)
		document.getElementById('theme'+Index).value = '';
}
...
function ProjektOption(Index,pNum,lNum,tNum, disabled, d1,d2,d3,d4,d5,d6,d7,theme,com,nbRows) {
	with (document) {
		writeln('<tr><td><SELECT NAME="projnr'+Index+'" id="projnr'+Index+'" '+
		...
		writeln('<SELECT NAME="lopnr'+Index+'" id="lopnr'+Index+'" '+disabled+' onChange="changeThemeFieldType('+Index+'); clear_statusbar(this.form)"  class=dropdown><OPTION></SELECT></td>');
		...
		writeln("<td><input type='text' "+disabled+" name='theme"+Index+"' id='theme"+Index+"' value='"+theme+"' "+
		        " class='theme' maxlength='16' onclick='if(activityIsAnEvol("+lNum+")) changeThemeFieldType("+Index+"); ' onfocus='clear_statusbar(this.form); Fokus2(this)' "+
...
function activityIsAnEvol(activiteId) {
	var liste_activite_evolution = new Array("2", "8", "12", "19", "22", "26", "29");
	
//	Bug Internet Explorer
//	Bout de code pour gérer la non prise en charge de indexOf par IE
	if(!Array.indexOf){  
	...
	return (liste_activite_evolution.indexOf(activiteId) > -1);
}

function changeThemeFieldType(index) {
	var themeName = "theme"+index;
	var eTheme = document.getElementById(themeName);
	var themeValue = eTheme.value;
	var eActivite = document.getElementById("lopnr"+index);
	
	if(activityIsAnEvol(eActivite.value)) {
//		c'est une évolution
		var eProjet = document.getElementById("projnr"+index);
		
		requestEvolList(ajaxRequestEvolList, eProjet.value, index, themeValue);
	} else {
//		ce n'est une évolution
		eTheme.outerHTML = 	"<input type='text' name='"+themeName+"' id='"+themeName+"' value='"+themeValue+"' "+
		        " class='theme' maxlength='16' onfocus='clear_statusbar(this.form); Fokus2(this)' "+
		        " onblur='LostFokus2(this)' />";
	}
}
...
function chooseWeek(iweek, form, weekMax)
{
	with (document) {
...
//function preFillRow(row, proj, akt, tid, d1,d2,d3,d4,d5,d6,d7,com,nbRows) {
...
function checkWeekData(e, counter) {
	var champ;
	var valeur, projet, theme, activite;
	
	if(counter <= 40) {
		champ = "document.table.projnr" + counter;
		projet = eval(champ+'.options['+champ+'.selectedIndex].value');
		if (projet != '0') {
			champ = "document.table.lopnr" + counter;
			activite = eval(champ+'.value');
			if (activite == '' || activite == '0' || activite == ' ') {
				alert("Remplir l'activit\351 de la ligne "+ counter);
				ichange = 1;
				return false;
			}
			champ = "document.table.theme" + counter;
			theme = eval(champ+'.value');
			if (theme == '' || theme == ' ') {
				alert('Remplir le thème de la ligne '+ counter);
				ichange = 1;
				return false;
			}
			champ = "document.table.commentaire" + counter;
			valeur = eval(champ+'.value');
			if (valeur == '' || valeur == ' ') {
				alert('Remplir le commentaire de la ligne '+ counter);
				ichange = 1;
				return false;
			}
//			A laisser à cette place car appel AJAX
			checkEvolId(e, projet, activite, theme, counter);
			checkWeekData(e, counter + 1);
		} else
			checkWeekData(e, counter + 1);
	} else
		f_checkbox_end(e);
...
function checkEvolId(e, projid, activiteid, evolid, ligne) {
//	if (activityIsAnEvol(activiteid))
		requestEvolID(ajaxCheckEvolId, e, projid, evolid, ligne, activiteid);
//	else
//		checkWeekData(e, ligne + 1);
	return;
}

function requestEvolID(callback, e, projid, evolid, ligne, activiteid) {
	var xhr = getXhr();
	// On défini ce qu'on va faire quand on aura la réponse
	xhr.onreadystatechange = function(){
		// On ne fait quelque chose que si on a tout reçu et que le serveur est ok
		if(xhr.readyState == 4 && xhr.status == 200)
			callback(xhr.responseText, e, evolid, ligne, activiteid);
	}

	xhr.open("POST", "../hrpic_tools/rodeo_ajax.php", true);
	// ne pas oublier ça pour le post
	xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	xhr.send("ajax_action=checkEvolId&projid="+projid+"&evolid="+evolid);
}

function requestEvolList(callback, projid, index, selectedValue) {
	var xhr = getXhr();
	// On défini ce qu'on va faire quand on aura la réponse
	xhr.onreadystatechange = function(){
		// On ne fait quelque chose que si on a tout reçu et que le serveur est ok
	...
	// ne pas oublier ça pour le post
	xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');
	xhr.send("ajax_action=requestEvolList&projid="+projid+"&selectedvalue="+selectedValue);
}

function ajaxCheckEvolId(status, e, evolid, ligne, activiteid) {
	if(activityIsAnEvol(activiteid)) {
		switch(status) {
			case '0':
//				checkWeekData(e, ligne + 1);
				...
				break;
			case '1':
				alert("Aucune évolution définie pour le client en ligne "+ligne);
				ichange = 1;
				stopEvent(e);
				return false;
				break;
			case '2':
				alert('Evolution "'+evolid+'" inconnue en ligne '+ligne);
				ichange = 1;
				stopEvent(e);
				return false;
				break;
			default:
				alert("Erreur inconnue : " + status);
				ichange = 1;
				stopEvent(e);
				return false;
				break;
		...
		switch(status) {
			case '0':
				alert("Attention, le thème "+evolid+" définit en ligne "+ligne+" semble être une évolution");
				return true;
				break;
			default:
				return true;
...
function ajaxRequestEvolList(listContent, index) {
	var themeName = "theme"+index;
	var eTheme = document.getElementById(themeName);
	var e1SelectContent = "<select name='"+themeName+"' id='"+themeName+"'>";
	
	e1SelectContent += "<option value =''>&nbsp;</option>";	
	e1SelectContent += listContent;	
	e1SelectContent += "</select>";
	eTheme.outerHTML = e1SelectContent;
}
