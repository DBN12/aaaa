...
my $opt_user     = 'rodeo_app_user';         # 'rodeo';
my $opt_password = '123RoDeO_UsEr/456' ;          # z:eyyzPNvEX3arHv
my $RELPATH = '../temps';
...
my %titles = (0=>'Salari&eacute;', 1=>'Responsable du Projet', 2=>'Responsable facturation', 3=>'Manager' );
...
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7);   # 2000,..,2016 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4);   # 2000,..,2018 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3);   # 2000,..,2019 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3,2);   # 2000,..,2020 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3,2,7);   # 2000,..,2021 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3,2,7,6);   # 2000,..,2022 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3,2,7,6,5);   # 2000,..,2023 <- num du 1er jeudi de janvier
#my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3,2,7,6,5,4);   # 2000,..,2024 <- num du 1er jeudi de janvier
my @dayOfJan1 = (6,4,3,2,1,6,5,4,3,1,7,6,5,3,2,1,7,5,4,3,2,7,6,5,4,2);   # 2000,..,2025 <- num du 1er jeudi de janvier
...
my @Months_W = ('- Mois -','Janvier','F&eacute;vrier','Mars','Avril','Mai','Juin','Juillet','Août','Septembre','Octobre','Novembre','D&eacute;cembre');
my @tidkoder = ('Normal', 'Extra1', 'Extra2', 'Non Factur&eacute;', 'Extra1 N.Fac.', 'Extra2 N.Fac.');

my @absent_vals = ('CP', 'Maladie', 'RTT', 'Ecole Terry', 'Sans solde', 'Absence pay&eacute;e', 'Mi-temps flexible', 'Komptid' );
...
HOPPA_INLOGGNING:
  ...
  if ( $logout || !&Valid ) {
    ...
    if    ( $logout )        { $login_message = "Vous &ecirc;tes d&eacute;connect&eacute;s. A bient&ocirc;t"; }
...
if ( !$lastfak ) {   # allra första gången han använder systemet
  ...
  if ( $week==$startweek && $year==$firstyear ) {
    $kommentar = "<b><font color=red>Votre premi&egrave;re semaine dans le syst&egrave;me</font></b><br/>";
    $firsttime = 1;
  }
  else {
    $kommentar = qq|<b>Votre date de d&eacute;but est $begindate<br/>Compl&eacute;ter la semaine <a class="a3" href="$scriptname?gotow=1&week=$startweek&year=$cur_year">$startweek</a></b><br/>|;
  }
}
else {
    ...
    # Remettre les compteurs &agrave; 0		
    $tyear = $year;
		$tweek = 0;
	}
...
$week = 1  if ($week < 1);
$week = $week_max if ($week > $week_max);
...
# GDA 20230412
$SQL = qq|Select distinct r.projid,r.theme,se.libelle
          From reports_$year r, suivi_evolution se
          Where (week = $week) And (userid = $user)
		    and r.projid = se.projid
			and r.theme = se.evolid
          Order By projid|;
&SQL_Execute($SQL);
my(%tabEvolSemaine);

while ( my @row = &SQL_Fetch() ) {
	$tabEvolSemaine{$row[0]."_".$row[1]} = $row[2];
}
...
if ($projects) {
	$SQL = qq|Select p.projid, c.name, p.description, activities_mask
	          ...
	          Where p.projid IN ($projects)
			    And (p.description not like '#%')
			    And (p.custid = c.custid) $null_filter
	          Order By c.name, p.description|;
	&SQL_Execute($SQL);

	while ( my($projnum, $projnamn, $beskrivning, $mask) = &SQL_Fetch() ) {
	  push(@projnr, $projnum);
	  my $projet_description = "$beskrivning ($projnum)";
	  my $proj_desc_length = length($projet_description);
	  while ($proj_desc_length++ < 25) {
		$projet_description .= "&nbsp;";
	  }
	  print "\t[$projnum, '$projet_description - $projnamn', $mask],\n";
	  $antal_projekt++;
	}
}
print "\t[400, 'Absent', 0]\n];\n";
...
print "<body onload=\"init_sums(document.table, $row_number);\" style=\" margin-left:30; background: url($RELPATH/mapfond-droite.png) bottom right no-repeat fixed\">\n";
...
print "<script type=\"text/javascript\"><!--\n";
print "chooseWeek($week, '', $week_max);\n";
print "--></script>\n";

print "  <select name=\"year\">\n";
# skriv ut year-select med hänsyn taget till anställningsdatum
for ($firstyear..$lastyear+1) {
  if ($_ == $year) { print "    <option selected=\"selected\">$year</option>\n"; }
  else             { print "    <option>$_</option>\n"; }
}
print "  </select>\n";

print "  <input type=\"submit\" name=\"gotow\" value=\"Aller &agrave; la sem.\" />\n";
my $forra = $week -1;
my $nasta = $week +1;
$forra = 1 if ($forra < 1);
$nasta = $week_max if ($nasta > $week_max);
...
if ( !$firsttime && $latest_week ) {
	print "<a href=\"#\" class=\"a3\" onClick=\"preFill()\">Pr&eacute;remplir comme la semaine $latest_week</a>\n";
}
...
if ($year == $tyear) {
	...
	if ($week <= $tweek) {
		...
		print "<p><b>$msg Cette semaine est marqu&eacute;e comme close. Prochaine semaine: <a class=\"a3\" href=\"$scriptname?gotow=1&week=$checka_week&year=$tyear\">$checka_week</a></b></p>\n";
	  ...
	  if ($year == $lastyear && $week == $lastweek) {
			print "<p><b><font color=#666666>Cloturer</font></b><input id=\"finished\" name=\"finished\" type=\"checkbox\" disabled=\"disabled\" /><i>Semaine actuelle!</i></p>\n";
		} else {
			print "<p><b>Cloturer</b><input type=\"checkbox\" id=\"finished\" name=\"finished\" value=\"1\" /></p>\n";
		}

	} else {
		print "\n<form method=\"post\" action=\"$scriptname?saveweek=1\" name=\"table\" target=\"transmitter\">\n";
#           print "<p><b>Derni&egrave;re semaine non close <a class=\"a3\" href=\"$scriptname?gotow=1&week=$checka_week&year=$tyear\">$checka_week</a></b></p>\n";
	}
	...
	print "<p><b>Derni&egrave;re semaine non close <a class=\"a3\" href=\"$scriptname?gotow=1&week=$tweek&year=$tyear\">$checka_week</a></b></p>\n";
}
if ($finished) {
	$disabled = 'disabled';
}
...
print "<a class=\"a3\" target=\"new\" href=\"../hrpic_tools/rodeo_to_hrpic.php?user_select=$user\" style=\"margin-left:20px\">Report mensuel</a> <b>pour saisie dans SAP</b>";
...
<table border="0" width="1250"><tr><td>
...
# GDA 20230419  
  my $evolLibelle = "";
  if ($lnr eq 2) {
	$evolLibelle = $tabEvolSemaine{$pnr."_".$theme};
	$evolLibelle =~ s/\\/\\\\/g;
	$evolLibelle =~ s/'/´/g;
  }
# /GDA 20230419
...
print qq|
  <tr>
    <td colspan="13"><img src="dummy.gif" width="1" height="8" alt="" /></td>
  </tr>
  <tr>
    <td colspan="3" align="right" class="td_r">Somme:</td>
    <td><input type="text" name="monday"    class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="tuesday"   class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="wednesday" class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="thursday"  class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="friday"    class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="saturday"  class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="sunday"    class="bsum" readonly="readonly" /></td>
    <td><input type="text" name="summa"     class="bsum" style="width:30px; color:red; font-size:13px; font-style:bold;" readonly="readonly" /></td>
    <td class="td_r" align="right" colspan="2">Heures restantes:<input type="text" name="tidbank" class="bsum" value="$tidbank" readonly="readonly" />[$tidbank]</td>
  </tr>
 <tr>
  <td align="center" colspan="13">
    ...
    <tr class="tr_header">
      ...
      <td width="100" class="insetBorder">&nbsp;Heures&nbsp;th&eacute;oriques:&nbsp;$arbetstid&nbsp;</td>
...
if (!$finished) {
  print "<input type=\"button\" name=\"saveweek\" value=\"Save\"   onClick=\"f_checkbox(event)\" />\n";
  print "<input type=\"button\" name=\"konsol\" value=\"Param&egrave;tres...\" onclick=\"return f_inst($week,$year);\" />\n";
...
print "<input type=\"button\" name=\"logout\" value=\"D&eacute;connexion\" onclick=\"LogOut($user);\" />";
  ...
  if ($password eq $password2) {
    ...
    if ( $oldpass eq crypt($oldpass2, $passSalt) ) {
      ...
      $SQL = qq|Update users
                ...
                Where userid = $user|;
      &SQL_Execute($SQL);
      $error_message = "<p>Mot de passe chang&eacute; !</p>\n";
    }
    else {
    ...
    $error_message = "<p>Mots de passe diff&eacute;rents !</p>\n";
  }
}
...
<p class=\"rubrik\">CBL Timereporting System - Param&egrave;tres</p>
<p class=\"rubrik\">$user - $EmpNames{$user}</p>
<p class=\"rubrik2\">Affect&eacute; au project</p>
...
foreach (@projs) {
  if ($_ == $project) { print "    <option selected=\"selected\" value=\"$project\">$project $kund{$project}</option>\n"; }
...
if ( $mensuel ) {
  ...
  for (0..12) {
    if ($_ == $month) { print "  <option selected=\"selected\" value=\"$month\">$Months_W[$month]</option>\n"; }
    else              { print "  <option value=\"$_\">$Months_W[$_]</option>\n"; }
  }
  print "</select>\n";
  print "Ann&eacute;e: <select name=\"year\">\n";
...
if ( $ofakturerad_period  && $behorig ) {
  ...
  print "<p>La p&eacute;riode choisie $period inclut des semaines d&eacute;j&agrave; factur&eacute;es.<br/>\n";
  print "La derni&egrave;re p&eacute;riode factut&eacute;e est $senaste_period cr&eacute;&eacute;e le $datum_skapad, o&ugrave; $belopp \€ [$timmar h] a &eacute;t&eacute; factur&eacute; par by $EmpNames{$signatur}.</p>\n";
}
else {
  if ( !$behorig ) {
    print "<p>Vous n'&ecirc;tes pas autoris&eacute; &agrave; facturer ce projet. Seul le Responsable de Projet le peut.</p>\n";
  }
  if ($belopp) {
    print "<p>La derni&egrave;re p&eacute;riode factut&eacute;e est  $senaste_period cr&eacute;&eacute;e le $datum_skapad, o&ugrave; $belopp \€ [$timmar h] a &eacute;t&eacute; factur&eacute; par $EmpNames{$signatur}.</p>\n";
  }
}
...
if ( !$valid_month ) {
  print "<br/><h3><font color=\"red\">Attention, la p&eacute;riode couvre plusieurs mois, le dernier tient lieu de r&eacute;f&eacute;rence pour les taux.</font></h3>\n";
}
...
print "  <td>Total/p&eacute;riode</td>\n";
print "</tr>\n";
...
while ( my($user, $week, $lopnr, $tidnr, $theme, $commentaire, @days) = &SQL_Fetch() ) {
  ...
  my $d1 = 1;
  my $d2 = 7;
# Correction du bug du jour d&eacute;cal&eacute; sur report mensuel
...
TA_HAND_OM_SISTA:
##
# Affichage de la colonne "Total/p&eacute;riode"
##
      ...
      print "<p style=\"text-align:left\"><font color=\"red\">$total heures &agrave; $arvo \€ $odeb_och_overtid</font><br/>";
...
##
# Section activit&eacute;
##
        if ($lnr_sum) {print "<br/><b>$lnamn ($lnr_sum)</b>"; }
##
# D&eacute;tail de l'activit&eacute; th&egrave;me par th&egrave;me
##
...
# Correction du bug de d&eacute;calage d'une journ&eacute;e
#          if    ($week == $week1) { $d1 = $day1 - 1; }
#          elsif ($week == $week2) { $d2 = $day2 - 1; }
          if    ($week == $week1) { $d1 = $day1; }
          elsif ($week == $week2) { $d2 = $day2; }
...
#    print "      <td ><b>Activit&eacute;</td>\n";
#    print "      <td width=\"30\"><b>Vu</td>\n";
#    print "      <td><b>Th&egrave;me</td>\n";
    print "      <td width=\"90px\"><b>Activit&eacute;</td>\n";
    print "      <td width=\"90px\"><b>Th&egrave;me</b></td>\n";
...
if ( $behorighet &&  $sparaemp) {
  ...
  if ($change = $form{'change'}) {
    ...
    $SQL = qq|Update users
              Set name = '$name', surname = '$surname', nick = '$nick', authority = $title, email = '$email', managerid = '$managerid', pernr = '$pernr'
    ...
    $cur_year = substr($startdatum,0,4);
    $SQL = qq|Insert Into users (userid, name, surname, nick, authority, password, begindate, timebank, current_year, email, managerid, pernr)
              Values ($empid, '$name', '$surname', '$nick', $title, '$password', '$startdatum', $tidbank, $cur_year, '$email', $managerid, $pernr)|;
...
print qq|<h2>Users
         ...
         <table border="0 "width="400"><tr><td class="td_spalt">Cliquer <i>userid</i> pour modifier le salari&eacute;.
...
if (!$change) {
  ...
  print "<tr class=tr_header>\n";
  print "  <td>userid</td>\n";
  print "  <td><a href=\"$scriptname?showemps=1&order=name\">Pr&eacute;nom</a></td>\n";
  print "  <td><a href=\"$scriptname?showemps=1&order=surname\">Nom</a></td>\n";
  print "  <td>Nick</td>\n";
  print "  <td>Matricule SAP</td>\n";
  print "  <td><a href=\"$scriptname?showemps=1&order=authority\">Autorisation</a></td>\n";
  print "  <td>Affect&eacute; &agrave;</td>\n";
  print "  <td>Timebank</td>\n";
  print "</tr>\n";

  $SQL = qq|Select userid, name, surname, nick, authority, timebank, pernr
            From users
            $filter
            Order By $order|;
  &SQL_Execute($SQL);

  while ( my ($idnr, $name, $surname, $nick, $title, $begin_tidbank, $pernr)  = &SQL_Fetch() ) {
    print "<tr>\n";
...
if ($change) {
  ...
  $SQL = qq|Select userid, name, surname, nick, authority, begindate, timebank, resigned, email, managerid, pernr
            From users
            Where userid = $change|;
  &SQL_Execute($SQL);
  @empch = &SQL_Fetch();
...
####################################################
  ...
  $SQL = qq|Select userid, CONCAT_WS(' ', name, surname) AS nom
              From users
			  WHERE authority > 0|;

  &SQL_Execute($SQL);
  my %liste_manager;
  while ( my ($managerId, $nomManager) = &SQL_Fetch() ) {
	$liste_manager{$managerId} = $nomManager;
  }
  $liste_manager{''} = "Autre";
...
if (!$change) {
  ...
  $empno_input = "<tr><td class=\"td_form\">Userid: <input type=\"text\" name=\"empid\" size=\"4\" maxlength=\"4\" class=\"ari2disabled\" value=\"$change\" disabled=\"disabled\" /></td>\n" .
...
<div id="$layer">
	...
	<tr>
		<td>
			...
			<tr>
				<td>&nbsp;<font color="white">$form_title</font></td>
				<td align="right" valign="middle">
					<a href=# onclick="helpWin('Employee')"><img src="$RELPATH/question.gif"width="16" height="16" border="0" alt="Help" /></a>
					<a href="#" onClick="javascript:hideLayer('$layer')"><img src="$RELPATH/kryss.gif" width="16" height="16" border="0" alt="St&auml;ng ned formul&auml;r" /></a>
				</td>
			</tr>
			</table>
			...
			<tr>
				$empno_input
				<td >Pr&eacute;nom:<br/><input type="text" name="name" size="15" value="$empch[1]" maxlength="30" class="ari2" onblur="clear_statusbar(this.form)"></td>
				...
				<td colspan="2" >Autorisation:<br/>
					<select name="title">
|;
					foreach my $titleId (sort keys %titles){
						print "<option $sel{$titleId} value=\"$titleId\">$titles{$titleId}</option>\n";
					}
print qq|					</select>
				</td>
			...
			<tr>
				<td >Date de d&eacute;but:<br/><input type="text" name="startdatum" $disabledate size="8" maxlength="8" value="$empch[5]" $disabledate maxlength="2" class="ari2" onblur="clear_statusbar(this.form)"></td>
				<td >Timebank:<br/><input type="text" name="tidbank" $disabledate size="3" value="$tidbank" maxlength="3" class="ari2" onblur="clear_statusbar(this.form)"></td>
				<td>Pernr<br /><input type="text" class="ari2" name="pernr" size="10" value="$empch[10]"></td>
			</tr>
			<tr>
				<td colspan="3">E-mail:<br/><input type="text" class="ari2" name="email" size="40" value="$empch[8]"></td>
			</tr>
			<tr>
				<td colspan="3">Manager:<br/>
					<select name="managerid">
|;
					foreach my $managerId (sort keys %liste_manager) {
						my $sel;
						if ( $managerId == $empch[9] ) {
							$sel = 'selected';
						}
						print "<option $sel value=\"$managerId\">$liste_manager{$managerId}</option>\n";
					}
print qq|
					</select>
				</td>
			...
			<tr>
				<td colspan="3" align="right"><input type="hidden" name="change" value="$change">
					<input type="submit" name="submit" value="OK"  onClick="submitData(event, validera_users())">
				</td>
			</tr>
			<tr>
...
print "<table border=\"0\" cellspacing=\"0\" cellpadding=\"4\" class=table_border_form>\n";
print "<tr><td>S&eacute;lectionner le salari&eacute; et l'ann&eacute;e pour g&eacute;n&eacute;rer le report</td></tr>\n";
print "<tr><td>Salari&eacute;:\n";
print "  <select name=\"empid\">\n";
foreach (@emps) {
  if ($_ == $empo) { print "    <option selected=\"selected\" value=\"$empo\">$EmpNames{$empo}</option>\n"; }
...
<body style="background: url($RELPATH/photo.jpg) bottom right no-repeat fixed; margin:0 0 0 30; margin-width:30"
>
...
<script type="text/javascript">
<!--$superUser
  writeMenu('red','showemps=1', "Salari&eacute;s", "Ajout/modif d&eacute;tails du salari&eacute;");
  writeMenu('red','kunder=1',   "Customers",    "Ajout/modif customers");
  writeMenu('red','projects=1', "Projets",   "Information du projets");
  writeMenu('red','lopnummer=1',"Activit&eacute;s", "Activit&eacute;s des projets");
  writeMenu('red','almanacka=1','Calendrier',  "heures hebdo annuelles, jours f&eacute;ri&eacute;s ...");

  writeMenu('green','reports=1','Time&nbsp;reports',   'Timereports');
  writeMenu('green','faktura=1','Temps&nbsp;&nbsp;projets',        'Temps pass&eacute;s par projets');
  writeMenu('green','allafakturor=1','Facturation', 'Paiements dûs/reçus pour comptabilit&eacute;');
  writeMenu('green','prognos=1','Pr&eacute;visions',        'Estimation des futures facturation et des heures travaill&eacute;es');

  writeMenu('red','logout=1', "D&eacute;connexion",         "D&eacute;gage !");
-->
</script>

    <td width="20"></td>
    <td  id="inloggad">Connect&eacute;:<br/>$name</td>
  </tr>
</table>

<!-- DIV style="margin-left:150">
<a href='?showemps=1'>Salari&eacute;s</a> 
<a href='?kunder=1'>Customers</a> 
<a href='?projects=1'>Projets</a> 
<a href='?lopnummer=1'>Activit&eacute;s</a> 
<a href='?almanacka=1'>Calendrier</a> 
<a href='?reports=1'>Time&nbsp;reports</a> 
<a href='?faktura=1'>Temps&nbsp;&nbsp;projets</a> 
<a href='?allafakturor=1'>Facturation</a> 
<a href='?prognos=1'>Pr&eacute;visions</a> 
...
print "<tr><td>Afficher les heures s&eacute;lectionn&eacute;es<br/> pour la facturation (tous projets)</td>\n";
...
#  Direkt ingång med knapp
  ...
  print "<h2>Aucunes factures pay&eacute;es\n";
...
print "  <td>Montant [\€]</td>\n";
print "  <td>Heures</td>\n";
print "  <td>Cr&eacute;&eacute;e&nbsp;par</td>\n";
print "  <td>Cr&eacute;&eacute;e</td>\n";
print "  <td>Envoy&eacute;e&nbsp;par</td>\n";
print "  <td>Envoy&eacute;e</td>\n";
...
if ($visaknapp && $behorighet) {
  print "<br/><input type=\"submit\" name=\"submit\" value=\"Marquer les entr&eacute;es comme envoy&eacute;es\" >\n";
...
#print "<p><a class=\"a3\" href=\"$scriptname?printer=1&week1=$week1&week2=$week2&year=$year\" target=\"new\">Ouvrir une version imprimable dans une nouvelle fen&ecirc;tre.</a></p>\n";
...
print "<table class=table_border border=\"0\" cellspacing=\"1\" cellpadding=\"2\">\n";
print "<tr class=tr_header>\n";
print "  <td rowspan=\"2\">Salari&eacute;s</td>\n";
print "  <td colspan=\"3\" align=\"center\">Derni&egrave;re ...</td>\n";
print "</tr>\n";
print "<tr class=tr_header>\n";
...
print "  <td align=\"center\" colspan=\"3\">Derni&egrave;re ...</td>\n";
print "</tr>\n";
print "<tr class=tr_header>\n";
print "  <td>sem. remplie</td>\n";
print "  <td>p&eacute;riode factur&eacute;e</td>\n";
print "  <td width=\"70\">cr&eacute;&eacute;e</td>\n";
print "</tr>\n";
...
for (1..$week_max) {
  if ($_ == $week1) { print "    <option selected=\"selected\" value=\"$_\">$_</option>\n"; }
...
if (!$noweek2) {
	...
	for (1..$week_max) {
	  if ($_ == $week2) { print "    <option selected=\"selected\" value=\"$_\">$_</option>\n"; }
	  else              { print "    <option value=\"$_\">$_</option>\n"; }
	}
	print "  </select>\n";
}
...
if ( $projlista eq '' ) {
  print "<h3>$EmpNames{$worker} n'a pas rempli l'ann&eacute;e $year</h3>";
  return;
}
...
# Projets & Activit&eacute;s Factur&eacute;s
print "\n<tr class=\"tr_bold\"><td>-Norm-</td><td colspan=\"$week_max\">&nbsp;</td></tr>\n";
...
# Projets & Activit&eacute;s Factur&eacute;s
print "\n<tr class=\"tr_bold\"><td>-OT, no charge-</td><td colspan=\"$week_max\">&nbsp;</td></tr>\n";
my $SQL = qq|Select projid, actid, week,monday+tuesday+wednesday+thursday+friday+saturday+sunday
             From reports_$year
             Where userid = $worker And timecode > 0|;
&SQL_Execute($SQL);
...
# Affichage du nombre d'heures ouvr&eacute;es (donc hors JF)
...
print "  <td bgcolor=\"white\"><b>Valid&eacute; par</b></td>\n";
print "  <td bgcolor=\"white\"><b>Fatur&eacute; par</b></td>\n";
...
print "<tr class=tr_header>\n";
print "  <td width=\"80\">Activit&eacute; id</td>\n";
print "  <td>Nom</td>\n";
print "  <td>Description</td>\n";
...
if (!$change) {
	print "<td colspan=\"2\"  >Nouvelle activit&eacute;<input type=hidden name=lopnr value=\"$maxlopnr\"></td>\n";
} else {
	print "<td colspan=\"2\"  >Modifier activit&eacute;: $change<input type=\"hidden\" name=\"lopnr\" value=\"$change\"></td>\n";
}
print "</tr><tr>\n";
...
print qq|
<h3>Affectation aux projets pour $user - $EmpNames{$user} ann&eacute;e $year</h3>
...
print "<tr><td>S&eacute;lectionner le projet et l'ann&eacute;e</td></tr>\n";
print "<tr><td>Projet:\n";
print "  <select name=\"projektdeltagare\" class=dropdown>\n";
while ( my($proj, $kund, $beskr) = &SQL_Fetch ) {
  if ($proj == $projektdeltagare) { print "    <option selected=\"selected\" value=\"$proj\">$proj $kund - $beskr</option>\n"; }
...
print qq|
<h2>Membres du projet pour $projektdeltagare - $kund $beskr ann&eacute;e $year
...
if ( $qform{ 'save' } ) {
  ...
  chop $reddays;
  $SQL = qq|Delete from workhours_$year
            Where week = $vecka|;
  &SQL_Execute($SQL);
  $SQL = qq|Insert INTO workhours_$year
			(week, hours, comment, freedays) values ('$vecka', '$arbetstid', '$kommentar', '$reddays')|;
  &SQL_Execute($SQL);
}
...
# Suite &agrave; bug 2007 (1er janvier = lundi).
# le deuxi&egrave;me my() fait RAZ des variables pr&eacute;c&eacute;demment d&eacute;clar&eacute;es
...
print "    <TD width=\"50\">Heures travaill&eacute;es</TD>\n";
print "    <TD>Commentaire</TD>\n";
...
<tr>
	<td>Th&egrave;me:<br/><textarea name="kommentar" rows="5" cols="40"></textarea></td>
</tr>
<tr>
...
<!--<body bgcolor="#FFFF99" style=leftmargin="0" topmargin="0" marginwidth="0">-->
<body>
...
<table border="0" cellspacing="0" cellpadding="3" class=table_form>
	<tr><td colspan="2">Observer les recettes pr&eacute;visionnelles en se basant sur la p&eacute;riode :</td></tr>
	<tr>
		<td colspan="2">
...
print qq|<h2>Pr&eacute;visions
  <a href="#" title="Help" onClick="helpWin('Prognos')"><img src="$RELPATH/help.gif" width="35" height="16" border="0" alt="Help" /></a></h2>
  <p>Revenus de r&eacute;f&eacute;rence: <font color=red>$summa</font> \€<br/>
     Revenus pr&eacute;visionnels: <font color=red>$f_totsum</font> \€<br/>
     $period $timmar heures ouvr&eacute;es</p>|;
...
<tr class=tr_header>
  ...
  <td colspan=2 align=center>Pr&eacute;visionnnel</td>
<tr>
<tr class=tr_header>
...
# 1&egrave;re semaine (w1), 1er jour (d1)
...
# derni&egrave;re semaine (w2), dernier jour (d2)
  $dagar += $months[$m] - 1;
...
#  print "<!--//pppppppppppppppppppppppppppp".$SQL."-->";
...
while ( my ($idnr, $name, $surname, $title, $inlogg) = &SQL_Fetch() ) {
  ...
  if (($title > 0) && ($title < 3)) { push (@ProjLeaders,$idnr); }
  if ($inloggade) {
...
while ( my ($idnr, $name, $surname, $title, $inlogg) = &SQL_Fetch() ) {
  ...
  $surname =~s/#$//g;
  $EmpNames{$idnr} = $name.'&nbsp;'.$surname;
  if ($title == 2) { push (@ProjLeaders,$idnr); }
  if ($inloggade) {
    $inloggad{$idnr} = $inlogg;
  }
}
...
$SQL = qq|Select week, freedays, hours, comment
          From workhours_$year|;
&SQL_Execute($SQL);
#@red_weeks[1..$week_max] = [];
#@arbetstid_weeks[1..$week_max] = 40;
...
#################################
    ...
    print "  <td width=\"100\">Activit&eacute;</td>\n";
    ...
    print "  <td width=\"70\">Th&egrave;me</td>\n";
    ...
    print "</tr></table>\n";
		...
		if    ($rapvecka{$week} > 0) {
		  print "<p>Semaine control&eacute;e par $fakvecka{$week}</p>\n"; }
		elsif ($rapvecka{$week} == -1 && !$user_role) {
			print "<p>Marquer la semaine comme control&eacute;e <input type=\"checkbox\" name=\"kollat$week\" value=\"1\"></p>\n";
		  $checkbolin = 1;
		}
		else {
			if ($rapvecka{$week} != -1) {
		  	print "<p><font color=\"red\">Semaine non finalis&eacute;e</font></p>\n";
			}
		}
...
if (!$user_role) {
	...
	if ($checkbolin  ) {
	  print "<input type=\"submit\" name=\"vidi\" value=\"Marquer les semaines comme control&eacute;e par $EmpNames{$admin}\" >\n";
	}
	print "</form>\n";
}
...
while ( my($week, $proj, $lopnr, $tidnr, $theme, @days) = &SQL_Fetch ) {
  ...
  if ($week != $oldweek) {
  	$oldweek = $week;
  	print qq|<tr class=tr_header><td colspan="6"><img src="dummy.gif" width="10" height="1" alt="dummy" /></td></tr>|;
  } elsif ($skip_first >1 ) {
  	$week = '';
  }
...
if ( $AccessRight == 0 ) {
  ...
  print "<script type=\"text/javascript\" src=\"$RELPATH/rodeo4.js\"></script>\n";

  if    ( $qform{'config'} ) {
    &UserConfig;
  }
  elsif ( $visa_year ) {
    print "<body bgcolor=\"#FFCC66\" style=\"margin-left:30\" >\n";
